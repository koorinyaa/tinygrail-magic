import { getUserAsks, UserBidsPageValue } from '@/api/user';
 ...
 */
export function MySellOrderDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
   ...
   */
  const fatchUserAsks = async (updateLoading: boolean = true) => {
    if (updateLoading) {
      setLoading(true);
    }

    try {
      const res = await getUserAsks(currentPage);
      if (res.State === 0) {
        setPageValue(res.Value);
      } else {
        throw new Error(res.Message ?? '获取卖单失败');
      }
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : '获取卖单失败';
      notifyError(errMsg);
    } finally {
      if (updateLoading) {
        setLoading(false);
      ...
      <DialogContent aria-describedby={undefined} className="p-4 rounded-xl">
        ...
        <div className="w-full h-fit flex flex-col gap-y-2 overflow-hidden">
          <div className="flex flex-row space-x-2">
            <h2 className="text-lg font-semibold">我的卖单</h2>
          </div>
