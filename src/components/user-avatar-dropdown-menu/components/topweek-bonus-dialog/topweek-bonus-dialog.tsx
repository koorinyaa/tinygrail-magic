 ...
 */
export function TopWeekBonusDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
   ...
   * 领取每周分红
   */
  const handleClaimWeeklyShareBonus = async () => {
    setLoading(true);
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : '领取每周分红失败';
      notifyError(errMsg);
    } finally {
      setLoading(false);
    }
        ...
        <div className="w-full h-fit flex flex-col gap-y-4">
          ...
          <div className="flex flex-row gap-x-2">
            <Button
              ...
              disabled={loading}
              onClick={async () => {
                await handleClaimWeeklyShareBonus();
                onOpenChange(false);
              }}
            >
              <LoaderCircleIcon
