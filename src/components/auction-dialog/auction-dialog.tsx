...
export function AuctionDialog({
  ...
  // 英灵殿数据
  const [tinygrailCharacterData, setTinygrailCharacterData] =
    useState<TinygrailCharacterValue>();
  const {
    // 参与拍卖人数
  ...
  useEffect(() => {
    if (!open) {
      setLoading(false);
      setTinygrailCharacterData(undefined);
      setCurrentAuctionInfo(undefined);
      setAmount(1);
      setPrice(0);
      setLock(false);
      setAlert({
   ...
   * 获取英灵殿数据
   */
  const fatchTinygrailCharacterData = async () => {
    if (!characterInfo.id) return;
    setLoading(true);

    try {
      const res = await getTinygrailCharacterData(characterInfo.id);
      if (res.State == 0) {
        setTinygrailCharacterData(res.Value);
        setPrice(Math.ceil(res.Value.Price * 100) / 100 || 0);
      } else {
        throw new Error(res.Message || '获取英灵殿数据失败');
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '获取英灵殿数据失败';
  ...
  const fetchAuctionList = async () => {
    ...
    try {
      const res = await getAuctionList([characterInfo.id]);
      if (res.State == 0) {
        if (
          res.Value.length > 0 &&
          res.Value[0].Id > 0 &&
          res.Value[0].Price > 0 &&
          res.Value[0].Amount > 0
        ) {
          setCurrentAuctionInfo(res.Value[0]);
          setAmount(res.Value[0].Amount);
          setPrice(res.Value[0].Price);
        }
      } else {
  ...
  const handleAuctionCharacter = async () => {
    ...
    try {
      ...
      const res = await auctionCharacter(characterInfo.id, amount, price);
      if (res.State === 0) {
        toast.success(res.Value || '参与竞拍成功');
        ...
        throw new Error(res.Message || '参与竞拍失败');
      }
    } catch (error) {
      const errorMessage =
  ...
  const handleCancelAuctionCharacter = async () => {
    if (!currentAuctionInfo || !characterInfo.id) return;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent
        ...
        className="p-4 rounded-xl sm:max-w-sm overflow-hidden"
      >
        {alert.show ? (
          ...
          <div className="w-full h-fit flex flex-col gap-y-2 overflow-hidden">
            ...
            <div className="flex flex-row items-center h-8 gap-x-1">
              <div className="w-12 text-sm opacity-60">出价</div>
              <InputNumber
                value={price}
                onChange={(value) => {
            ...
            <div className="flex flex-row items-center h-8 gap-x-1">
              <div className="w-12 text-sm opacity-60">数量</div>
              <InputNumber
                value={amount}
                onChange={(value) => {
