...
import { StardustContent } from '@/components/stardust-content';
...
import { fetchUserCharacterData, onTemplesChange } from '../../service/user';

/**
 * 星光碎片
 * @param {() => void} onClose 关闭回调
 */
export function Stardust({ onClose }: { onClose: () => void }) {
  const {
    userAssets,
    setCharacterDrawer,
    characterDrawerData,
  ...
  // 上次选择
  const [lastStardust, setLastStardust] = useState<CharacterDetail | null>(
    localStorage.getItem('tinygrail-magic:lastStardust')
      ? (JSON.parse(
          localStorage.getItem('tinygrail-magic:lastStardust') as string
        ) as CharacterDetail)
      : null
  );
  // 选中角色
   ...
   * 更新上次选择角色信息
   */
  const updateLastStardust = async () => {
    if (!userAssets || !lastStardust) return;

    try {
      const characterDetailData = await fetchCharacterDetailData(
        lastStardust.CharacterId
      );
      ...
      if ('Current' in characterDetailData) {
        setLastStardust({
          ...characterDetailData,
          UserAmount: userCharacterData.Amount,
        });
      }
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : '获取角色详情失败';
      console.error(errMsg);
      setLastStardust(null);
      localStorage.removeItem('tinygrail-magic:lastStardust');
    }
  };
    ...
    >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-x-2 h-4 text-xs opacity-60">
          <span className="truncate">
            可用持股：{formatInteger(character.UserAmount)}
          </span>
        </div>
      </div>
    </div>
      ...
      <div className="flex flex-col">
        {lastStardust && (
          <>
            ...
            <div>
              <CharacterItem character={lastStardust} />
            </div>
          </>
        )}
        <p className="text-xs opacity-60 py-2">请选择星光碎片消耗的目标</p>
        <div className="flex flex-col">
